...
function poolCanvasRenderer(canvas, cfg) {
  ...
  function frame() {
    ...
    if (cfg.showPres) {
      ctx.beginPath(); ctx.arc(567,162,24,0,Math.PI*2); ctx.fillStyle="#0d1a12"; ctx.fill(); ctx.strokeStyle="#1d6633"; ctx.lineWidth=1.5; ctx.stroke();
      ctx.beginPath(); ctx.arc(567,162,20,0,Math.PI*2); ctx.strokeStyle="#154422"; ctx.lineWidth=1; ctx.stroke();
      ctx.fillStyle="#44bb77"; ctx.font="9px sans-serif"; ctx.textAlign="center"; ctx.fillText("bar",567,157);
      ctx.fillStyle="#22dd66"; ctx.font="bold 14px sans-serif"; ctx.fillText(cfg.pres,567,172);
    }
